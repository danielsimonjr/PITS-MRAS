...
- **Deliverables** [IP PAGES 38–40, §8.1–§8.3]:
  ...
  - `src/pits_mras/training/cotrain.py` — `cotraining_loop`; "the most critical training file." Extends Algorithm 3. PITNN objective `L_total` (physics + optional PCML + `0.1·L_cbf`) on `optimizer_pitnn` (Adam lr=1e-4); critic-only updates on a separate `critic_optimizer` (Adam lr=1e-3): opt-in HJB regularizer (if `λ_hjb>0`, default 0), guarded critic positivity `1e-3·L_pos`, and the IRL Bellman step (grad-clip 1.0) + policy improvement `K_new=R⁻¹BᵀP̂`. (v0.4.0 removed the vacuous costate-consistency term — Identity 2 holds by construction.) [IP PAGES 39–40, §8.2]
...
- **G1 — Blueprint vs Plan "three new heads" — PARTLY RESOLVED (v0.4.5).** Blueprint says critic / **disturbance-adversary** / CBF [BP PAGE 1]; the Plan built critic / **costate** / CBF. v0.4.5 added the **analytic H∞ core** (`solve_gare` + `AdversaryHead`, `w*=γ⁻²DᵀPe` by construction), so the adversary head now exists alongside costate. Remaining: the neural min-max training loop (v0.5.0); SAC/entropy (Connection 5) and TD-MPC2 (Connection 9) still have no dedicated modules. [BP PAGE 1,6 vs IP §5,§7]
